...
Weighted operator-=(Weighted &first, Weighted &second)
{
    ...
    for (auto e: second_edg) {
        auto place = std::find(first.edges.begin(), first.edges.end(), e);
        ...
        place = std::find(first.edges.begin(), first.edges.end(), std::make_pair(e.second, e.first));
        if (place != first.edges.end()) {
            first.edges.erase(place);
            int index = place - first.edges.begin();
            first.weights.erase(first.weights.begin() + index);
...
Weighted operator-(Weighted &first, TGraph &second)
{
    ...
    for (auto e: second_edg) {
        auto place = std::find(first_edg.begin(), first_edg.end(), e);
        if (place != first_edg.end()) {
            first_edg.erase(place);
            int index = place - first_edg.begin();
            first_weights.erase(first_weights.begin() + index);
        }
        place = std::find(first_edg.begin(), first_edg.end(), std::make_pair(e.second, e.first));
        if (place != first_edg.end()) {
            first_edg.erase(place);
            int index = place - first_edg.begin();
            first_weights.erase(first_weights.begin() + index);
    ...
    for (auto e: first_edg) {
        std::stringstream ss;
        ss << e.first << e.second;
        new_edges.push_back(ss.str());
    }
    return Weighted(new_edges, first_weights);
}
...
Bipartite operator-=(Bipartite &first, Bipartite &second)
{
    ...
    auto second_p2 = second.GetPart2();
    for (auto v: second_p1) {
        auto place = std::find(first.part1.begin(), first.part1.end(), v);
        if (place != first.part1.end()) {
            first.part1.erase(place);
        }
    }
    for (auto v: second_p2) {
        auto place = std::find(first.part2.begin(), first.part2.end(), v);
...
Complete operator-=(Complete &first, Complete &second)
{
    auto second_v = second.GetVertices();
    for (auto v: second_v) {
        auto place = std::find(first.vertices.begin(), first.vertices.end(), v);
